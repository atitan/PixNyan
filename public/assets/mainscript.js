var previous_replyhlno=0,arrPresetFunc=[],arrSakuraTbl=[[63223,12353,82],[63306,12449,85],[63486,12535,4]],arrSakuraTblsp=[[63216,63219,63210,63211,63212,63213],[12293,12540,12541,12542,12445,14446]];
function $g(a){
...
function getCookie(a){
	...
	for(var h=d.length;e<h;){
		...
		b=d.substring(e+1,g);
		f=b.indexOf("=");
		if(b.substring(0,f)===a)
			return window.unescape(b.substring(f+1,g-e-1));
		e=g+1
	}
	return""
}
...
function replace_sakura(a){
	var d=a.value,b=0,e=0,g=arrSakuraTbl.length,f;
	for(b=0;b<g;b++){
		f=arrSakuraTbl[b];
		for(e=0;e<=f[2];e++)
			d=d.replace(new RegExp(String.fromCharCode(f[0]+e),"g"),String.fromCharCode(f[1]+e))
	}
...
function l1(){
	var a=getCookie("namec"),d=getCookie("emailc"),b;
	if(b=$g("post_author"))
		b.value=a;
	if(b=$g("post_email"))
		b.value=d
...
function c(){
	...
	try{
		if(!$g("post_image"))
			return true;
		a=$g("post_image").value;
		if(!a&&!$g("post_message").value){
			alert(msgs[0]);
			return false
		}
		if(a){
			...
			check_sakura("post_message");
			check_sakura("post_author");
			check_sakura("post_title");
			if(window.clipboardData)
				document.forms[0].upfile_path.value=a;
			document.forms[0].sendbtn.disabled=true
	}catch(g){

	}
	$g("post_author").value&&setCookie("namec",$g("post_author").value)
}
	...
	if(document.getElementsByTagName)
		for(var a,d=document.getElementsByTagName("a"),b=d.length,e=0;e<b;e++){
			...
			if(a.getAttribute("href")){
				if(a.getAttribute("rel")==="_top")
					a.target="_top";
				if(a.getAttribute("rel")==="_blank")
					a.target="_blank"
			}
		}
	}
...
function hideform(){
	...
	try{
		$g("post_message").focus();
		$g("post_message").value+=">>No."+a+"\r\n"
	}catch(d){
...
function replyhl(a,d){
	...
	if(b)
		if(d)
			b.className=b.className.replace(" reply_hl","");
		else{
...
function preset(){
	var a,d=arrPresetFunc.length,b;
	fixalllinks();
	for(a=0;a<d;a++){
		b=arrPresetFunc[a];
		typeof b==="function"&&b()
